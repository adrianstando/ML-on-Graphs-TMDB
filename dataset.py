...
class TMDBDataset(torch_geometric.data.InMemoryDataset):
    ...
    def __init__(self, root: str = './tmp', node_feature_method: Literal['counter', 'tf-idf'] = 'counter',
                 ...
                 jaccard_distance_threshold: Optional[float] = None):
        """
        The Graph Dataset representing the TMDB data.
    ...
    def raw_file_names(self):
        return ['tmdb_5000_credits.csv', 'tmdb_5000_movies.csv']
    ...
    def _extract_edge_weights(self, df):
        ...
        # remove mirror pairs
        df_cross = df_cross.loc[pd.DataFrame(
            np.sort(df_cross[['id_x', 'id_y']], 1), index=df_cross.index
        ).drop_duplicates(keep='first').index]

        # calculate distances
        df_cross['distance'] = df_cross.apply(lambda x: self.jaccard_similarity(
            np.unique(x['people_data_x'].split(' ')),
            np.unique(x['people_data_y'].split(' '))
        ), axis=1)
